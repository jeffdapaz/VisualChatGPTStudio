...
- **添加注释：** 为选定的代码添加注释。
- **添加摘要：** 为 C# 方法添加摘要。
- **添加整个类的摘要：** 为整个 C# 类添加摘要（适用于方法、属性、枚举、接口、类等）。无需选择代码，只需运行命令以启动该过程。
- **提问：** 在代码编辑器中编写一个问题，然后等待答案。
- **翻译：** 用翻译后的版本替换选定的文本。在选项窗口中编辑命令，如果想要将其翻译成英语以外的其他语言。
- **自定义前置：** 通过选项创建一个自定义命令，在选定的代码之前插入响应。
- **自定义后置：** 通过选项创建一个自定义命令，在选定的代码之后插入响应。
- **自定义替换：** 通过选项创建一个自定义命令，用响应替换选定的文本。
