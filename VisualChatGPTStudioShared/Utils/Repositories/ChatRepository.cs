    ...
    public static class ChatRepository
    {
        ...
        public static void CreateDataBase()
        {
            try
            {
                ...
                CreateTableMcpServerDefinitions();
            }
            catch (Exception ex)
            {
                Logger.Log(ex);
        ...
        /// <summary>
        /// Creates the MCP_SERVER_DEFINITIONS table in the database if it does not already exist.
        /// </summary>
        private static void CreateTableMcpServerDefinitions()
        {
            string query = @"CREATE TABLE IF NOT EXISTS MCP_SERVER_DEFINITIONS
                            (
                                ID          VARCHAR(50)  PRIMARY KEY UNIQUE NOT NULL,
        ...
        public static void DeleteChat(string chatId)
        {
            ...
            DeleteMcpServerDefinitions(chatId);
        }
        ...
        /// <summary>
        /// Retrieves a list of MCP server names associated with a specific chat ID from the database.
        /// </summary>
        /// <param name="chatId">The chat ID used to filter the MCP server names.</param>
        /// <returns>
        /// A list of MCP server names as strings corresponding to the provided chat ID.
        /// </returns>
        ...
        /// <summary>
        /// Adds a new MCP server definition to the database by inserting a record with a unique ID, chat ID, and server name.
        /// </summary>
        /// <param name="chatId">The chat identifier.</param>
        /// <param name="serverName">The MCP server name.</param>
        public static void AddMcpServerDefinition(string chatId, string serverName)
        {
            ...
            command.Bind("@id", Guid.NewGuid());
            command.Bind("@chatId", chatId);
            command.Bind("@serverName", serverName);

            command.ExecuteNonQuery();
        }

        /// <summary>
        /// Deletes MCP server definitions from the database based on the provided chat ID.
        /// </summary>
        /// <param name="chatId">The chat identifier.</param>
